...
class SampleSnapshot:
    ...
    def setData(self, samplesnap: str, actions: list[Action]) -> 'SampleSnapshot':
...
def checkSampleSnapshot(sampleSnap: SampleSnapshot, buttons: list[Button]) -> None:
    expected = set(sampleSnap.actions)
    actual = set()
    for button in buttons:
        snapButton = sampleSnap.snap.crop((button.x, button.y, button.x + button.width, button.y + button.height))
